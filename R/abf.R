...
##     print(family)
  ...
  if(is.null(parallel.dir)) { # run it
    ...
    results <- lapply(seq_along(snps), function(i) {
      ...
      k0=ncol(qm)
      k=length(snps[[i]]) + k0
      model <- speedglm.wfit1(y2, cbind(x2[, snps[[i]], drop=FALSE ],qm), family=family)
      model0 <- speedglm.wfit1(y2, qm, family=family)
...
##' Internal adaption of speedglm function speedglm.wfit1 due to dependency issue.  See speeglm package for thorough documentation.
##'
##' @title speedglm.wfit1
##' @author Jamie Inshaw
##' @export

speedglm.wfit1 <- function(y, X, intercept=TRUE, weights=NULL,row.chunk=NULL,
  ...
  ll.nuovo <- ll.speedglm1(fam,aic.model,rank)
  res <- (y-mu)/mu.eta(eta)
  resdf <- n.ok - rank
  RSS <- sum(W*res*res)
  var_res <- RSS/dfr  
...
ll.speedglm1 <- function(family,aic.model,nvar){
  switch(family,
